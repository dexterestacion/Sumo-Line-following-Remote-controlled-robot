...
the second arduino(main) will handle the entire sensors in the mobot and the Motor driver, and uses the commands from the first arduino to control the modes via RC tranceiver.
 ...
 the RC tranceiver is a digital tranceiver which consists of 6 buttons for transmission and 6 output(M1,M2, and M3) to receiver. the output of this tranceiver will be received by the first arduino uno to be used for mode selection and RC. 
